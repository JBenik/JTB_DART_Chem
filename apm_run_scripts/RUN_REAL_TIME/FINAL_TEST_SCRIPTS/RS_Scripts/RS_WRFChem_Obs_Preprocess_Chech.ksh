...
#	2. Check that index files exists
#	3. Check that there are 3 index files
...
#	8. Check that the wrfchemi files are present
#	9. Check that the obs_seq_comb_filtered files are present
...
cd "${RUN_DIR}/${DATE}/preprocess_obs" || abort "ERROR: Could not find or access ${RUN_DIR}/${DATE}/preprocess_obs directory."

# Checking for index files in chem_emiss
if [ ! -e index_nco_1.html ] || [ ! -e index_prepr.html ] || [ ! -e index_preprocess.html ]; then
...
if [[ "$index_count" -ne 3 ]]; then
    abort "Missing an index file, please check preprocess_obs"
fi
...
# Checking if obs_seq_comb_filtered exists. 
if ! ls obs_seq_comb_filtered* >/dev/null 2>&1; then
    abort "ERROR: No obs_seq_comb_filtered files found, exiting."
fi
