...
while [[ ${MEM} -le ${NUM_MEMBERS} ]]; do
   ...
   # Check wrfchemi
   if ! ncdump -h wrfchemi_d${CR_DOMAIN}_${LL_FILE_DATE}_filt.${CMEM} | grep -q emissions_zdim; then
        echo "JTB: ERROR: Missing emissions_zdim in wrfchemi file ${CMEM}"
        let x=1 
   fi  

   # Check wrffirechemi
   if ! ncdump -h wrffirechemi_d${CR_DOMAIN}_${LL_FILE_DATE}_filt.${CMEM} | grep -q emissions_zdim_stag; then
        echo "JTB: ERROR: Missing emissions_zdim_stag in wrffirechemi file ${CMEM}"
        let x=1 
   fi  

   # Check WRFCHEMI DARTVARS
   CHEMI_DARTVARS_SearchString="${WRFCHEMI_DARTVARS//,/|}"
   if ! ncdump -h wrfout_d${CR_DOMAIN}_${FILE_DATE}_filt.${CMEM} | grep -qE "${CHEMI_DARTVARS_SearchString}"; then
   ...
   if ! ncdump -h wrfout_d${CR_DOMAIN}_${FILE_DATE}_filt.${CMEM} | grep -qE "${FIRECHEMI_DARTVARS_SearchString}"; then
